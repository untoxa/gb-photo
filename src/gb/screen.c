#include <gbdk/platform.h>
#include <stdint.h>

#include "screen.h"
#include "gbcamera.h"
#include "vwf.h"

#define TO_TILE_ADDRESS(BASE, NO) ((BASE) + ((NO) << 4))

uint8_t * const screen_tile_addresses[18] = {
    TO_TILE_ADDRESS(_VRAM8800, 0x80), 
    TO_TILE_ADDRESS(_VRAM8800, 0x94),
    TO_TILE_ADDRESS(_VRAM8800, 0xA8), 
    TO_TILE_ADDRESS(_VRAM8800, 0xBC), 
    TO_TILE_ADDRESS(_VRAM8800, 0xD0), 
    TO_TILE_ADDRESS(_VRAM8800, 0xE4),
    TO_TILE_ADDRESS(_VRAM8800, 0x00),   
    TO_TILE_ADDRESS(_VRAM8800, 0x14),  
    TO_TILE_ADDRESS(_VRAM8800, 0x28),  
    TO_TILE_ADDRESS(_VRAM8800, 0x3C),
    TO_TILE_ADDRESS(_VRAM8800, 0x50),  
    TO_TILE_ADDRESS(_VRAM8800, 0x64), 
    TO_TILE_ADDRESS(_VRAM8000, 0x00), 
    TO_TILE_ADDRESS(_VRAM8000, 0x14),   
    TO_TILE_ADDRESS(_VRAM8000, 0x28),  
    TO_TILE_ADDRESS(_VRAM8000, 0x3C),
    TO_TILE_ADDRESS(_VRAM8000, 0x50),  
    TO_TILE_ADDRESS(_VRAM8000, 0x64)
};

const uint8_t screen_tile_map[360] = {
     
};

const uint8_t screen_tiles[] = {
    0x00,0x00,0x7D,0x7D,0x40,0x40,0x40,0x40,0x40,0x40,0x40,0x40,0x00,0x00,0x40,0x40,
    0x00,0x00,0xBE,0xBE,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x00,0x00,0x02,0x02,
    0x02,0x02,0x00,0x00,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0xBE,0xBE,0x00,0x00,
    0x40,0x40,0x00,0x00,0x40,0x40,0x40,0x40,0x40,0x40,0x40,0x40,0x7D,0x7D,0x00,0x00,
    0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
    0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF
};

void screen_init() {
    set_data(TO_TILE_ADDRESS(_VRAM8000, 256 - (sizeof(screen_tiles) >> 4)), screen_tiles, sizeof(screen_tiles));
}

void screen_load_image(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint8_t * picture) {
    static uint8_t **addr, *data, i;

    data = picture;
    addr = (uint8_t **)(screen_tile_addresses + y);

    for (i = 0; i != h; i++, data += (w << 4))
        set_data(*addr++ + (x << 4), data, (w << 4));
}

uint8_t screen_show_picture(uint8_t n) {
    wait_vbl_done();
    screen_clear_rect(IMAGE_DISPLAY_X, IMAGE_DISPLAY_Y, CAMERA_IMAGE_TILE_WIDTH, CAMERA_IMAGE_TILE_HEIGHT, SOLID_BLACK);

    if (n > (CAMERA_MAX_IMAGE_SLOTS - 1)) return FALSE;
    SWITCH_RAM((n >> 1) + 1);
    screen_load_image(IMAGE_DISPLAY_X, IMAGE_DISPLAY_Y, CAMERA_IMAGE_TILE_WIDTH, CAMERA_IMAGE_TILE_HEIGHT, ((n & 1) ? image_second : image_first));

    wait_vbl_done();
    screen_restore_rect(IMAGE_DISPLAY_X, IMAGE_DISPLAY_Y, CAMERA_IMAGE_TILE_WIDTH, CAMERA_IMAGE_TILE_HEIGHT);
    return TRUE;
}

typedef struct thumb_coord_t {
    uint8_t x, y;
} thumb_coord_t;

const thumb_coord_t screen_thumbnail_coords[MAX_PREVIEW_THUMBNAILS] = {
    {2,  1}, {6,  1}, {10,  1}, {14,  1},
    {2,  5}, {6,  5}, {10,  5}, {14,  5},
    {2,  9}, {6,  9}, {10,  9}, {14,  9},
    {2, 13}, {6, 13}, {10, 13}, {14, 13}

};

uint8_t screen_show_tumbnails(uint8_t start) {
    wait_vbl_done();
    screen_clear_rect(THUMBNAIL_DISPLAY_X, THUMBNAIL_DISPLAY_Y, THUMBNAIL_DISPLAY_WIDTH, THUMBNAIL_DISPLAY_HEIGHT, SOLID_BLACK);
    static uint8_t i, j;
    j = start;
    for (i = 0; i != 16; i++, j++) {
        if (j > (CAMERA_MAX_IMAGE_SLOTS - 1)) break;

        SWITCH_RAM((j >> 1) + 1);
        screen_load_image(screen_thumbnail_coords[i].x, screen_thumbnail_coords[i].y, CAMERA_THUMB_TILE_WIDTH, CAMERA_THUMB_TILE_HEIGHT, ((j & 1) ? image_second_thumbnail : image_first_thumbnail));

        wait_vbl_done();
        screen_restore_rect(screen_thumbnail_coords[i].x, screen_thumbnail_coords[i].y, CAMERA_THUMB_TILE_WIDTH, CAMERA_THUMB_TILE_HEIGHT);
    }
    return TRUE;
}

uint8_t screen_text_render(uint8_t x, uint8_t y, const uint8_t * text) {
    return vwf_draw_text(screen_tile_addresses[y] + (x << 4), text);
}

uint8_t screen_text_out(uint8_t x, uint8_t y, const uint8_t * text) {
    uint8_t len = vwf_draw_text(screen_tile_addresses[y] + (x << 4), text);
    screen_restore_rect(x, y, len, 1);
    return len;
}