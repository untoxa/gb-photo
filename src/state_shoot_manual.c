...
static void refresh_screen() {
    ...
    screen_text_out(0, 0, "\x03\xff Manual mode");
}

uint8_t ENTER_state_shoot_manual() BANKED {
    refresh_screen();
    return 0;     
}

uint8_t UPDATE_state_shoot_manual() BANKED {
    PROCESS_INPUT();
...
uint8_t LEAVE_state_shoot_manual() BANKED {
    return 0;     
}
